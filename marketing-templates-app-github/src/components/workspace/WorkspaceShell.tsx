...
import {
  Archive, Bell, CalendarDays, ChartNoAxesCombined, Check, CheckCircle2, ChevronDown, ChevronRight,
  Circle, Clapperboard, Command, File, FileText, FolderKanban, Home, Languages,
  LayoutDashboard, Lightbulb, ListTodo, LogOut, Menu, Moon, MoreHorizontal,
...
function ModuleLoader() {
  return <div className="ws-module-loading"><span />Loading marketing workspace…</div>;
}

export type ProjectSection = 'overview' | 'tasks' | 'board' | 'timeline' | 'content' | 'files' | 'ideas';
...
interface WorkspaceShellProps {
  ...
  onOpenIdeas: () => void;
  onUploadFile: (file: File, link?: Pick<WorkspaceDocument, 'planId' | 'taskId' | 'clientId' | 'campaignId' | 'contentId' | 'ideaId'>) => Promise<WorkspaceDocument | null>;
  ...
  onConvertIdeaToContent: (ideaId: string) => void;
...
export default function WorkspaceShell(props: WorkspaceShellProps) {
  const {
    ...
    projects, archivedProjects, archivedProjectIds, items, ideas, documents, clients, campaigns, contentItems, metrics, socialAccounts, remindersCount,
    ...
    onOpenTeam, onToggleTheme, onToggleLanguage, onSignOut, onCreateContent, onUpdateContent,
    onDeleteContent, onRepurposeContent, onCreateClient, onCreateCampaign, onAddMetric,
    onUpdateAccount, onConvertIdeaToContent, onConvertIdeaToTask,
  } = props;
  ...
  const searchResults = useMemo(() => {
    if (!deferredQuery) return { clients: [], projects: [], campaigns: [], tasks: [], content: [], ideas: [], files: [] };
    return {
      clients: clients.filter(client => `${client.name} ${client.website || ''}`.toLocaleLowerCase().includes(deferredQuery)).slice(0, 5),
      projects: projects.filter(project => `${project.title} ${project.category}`.toLocaleLowerCase().includes(deferredQuery)).slice(0, 5),
      campaigns: campaigns.filter(campaign => `${campaign.name} ${campaign.objective}`.toLocaleLowerCase().includes(deferredQuery)).slice(0, 5),
      ...
      content: contentItems.filter(item => `${item.title} ${item.caption || ''} ${item.platforms.join(' ')}`.toLocaleLowerCase().includes(deferredQuery)).slice(0, 8),
  ...
  }, [campaigns, clients, contentItems, deferredQuery, documents, ideas, items, projects]);
  ...
  const renderToday = () => (
    <div className="ws-page ws-today">
      ...
      <div className="ws-metrics">
        {[[overdue.length, 'Прострочено', 'Overdue'], [dueToday.length, 'На сьогодні', 'Due today'], [inProgress.length, 'У роботі', 'In progress'], [inReview.length + contentItems.filter(item => item.status === 'review').length, 'Очікує перевірки', 'Waiting review'], [scheduledToday.length, 'Контент сьогодні', 'Scheduled content']].map(([value, uk, en], index) => <div className={index === 0 && Number(value) > 0 ? 'danger' : ''} key={String(uk)}><strong>{value}</strong><span>{lang === 'uk' ? uk : en}</span></div>)}
      </div>
  ...
  const renderProject = () => {
    ...
    return <div className="ws-page ws-project-page">
      ...
      <div className="ws-project-meta"><span><Users size={15} />{activeProject.owner || accountEmail}</span><span><CalendarDays size={15} />{activeProjectData?.startDate} — {activeProjectData?.endDate}</span><span><Progress value={activeProjectData?.progress || 0} />{activeProjectData?.progress || 0}%</span></div>
      <nav className="ws-project-tabs" aria-label={lang === 'uk' ? 'Розділи проєкту' : 'Project sections'}><select value={section} onChange={event => setSection(event.target.value as ProjectSection)}>{(['overview', 'tasks', 'board', 'timeline', 'content', 'files', 'ideas'] as const).map(value => <option value={value} key={value}>{value === 'overview' ? (lang === 'uk' ? 'Огляд' : 'Overview') : value === 'tasks' ? (lang === 'uk' ? 'Завдання' : 'Tasks') : value === 'board' ? (lang === 'uk' ? 'Дошка' : 'Board') : value === 'timeline' ? 'Timeline' : value === 'content' ? (lang === 'uk' ? 'Контент' : 'Content') : value === 'files' ? (lang === 'uk' ? 'Файли' : 'Files') : (lang === 'uk' ? 'Ідеї' : 'Ideas')}</option>)}</select>{(['overview', 'tasks', 'board', 'timeline', 'content', 'files', 'ideas'] as const).map(value => <button className={section === value ? 'active' : ''} onClick={() => setSection(value)} key={value}>{value === 'overview' ? (lang === 'uk' ? 'Огляд' : 'Overview') : value === 'tasks' ? (lang === 'uk' ? 'Завдання' : 'Tasks') : value === 'board' ? (lang === 'uk' ? 'Дошка' : 'Board') : value === 'timeline' ? 'Timeline' : value === 'content' ? (lang === 'uk' ? 'Контент' : 'Content') : value === 'files' ? (lang === 'uk' ? 'Файли' : 'Files') : (lang === 'uk' ? 'Ідеї' : 'Ideas')}</button>)}</nav>
      ...
      {section === 'content' ? <Suspense fallback={<ModuleLoader />}><MarketingModules mode="content" lang={lang} projects={projects} clients={clients} campaigns={campaigns} contentItems={contentItems.filter(item => item.projectId === activeProjectId)} documents={documents} metrics={metrics} accounts={socialAccounts} initialProjectId={activeProjectId} onCreateContent={onCreateContent} onUpdateContent={onUpdateContent} onDeleteContent={onDeleteContent} onRepurposeContent={onRepurposeContent} onCreateClient={onCreateClient} onCreateCampaign={onCreateCampaign} onAddMetric={onAddMetric} onUpdateAccount={onUpdateAccount} onUploadFile={onUploadFile} onOpenFile={onOpenFile} onDeleteFile={onDeleteFile} onOpenSettings={() => navigate('settings')} /></Suspense> : null}
  ...
  const moduleProps = { lang, projects, clients, campaigns, contentItems, documents, metrics, accounts: socialAccounts, onCreateContent, onUpdateContent, onDeleteContent, onRepurposeContent, onCreateClient, onCreateCampaign, onAddMetric, onUpdateAccount, onUploadFile, onOpenFile, onDeleteFile, onOpenSettings: () => navigate('settings') };
  const marketingModule = (mode: 'content' | 'analytics' | 'settings') => <Suspense fallback={<ModuleLoader />}><MarketingModules mode={mode} {...moduleProps} /></Suspense>;
  const content = activeTab === 'home' ? renderToday() : activeTab === 'plans' ? renderProjects() : activeTab === 'all_tasks' ? renderTasks() : activeTab === 'content' ? marketingModule('content') : activeTab === 'analytics' ? marketingModule('analytics') : activeTab === 'settings' ? marketingModule('settings') : activeTab === 'calendar' ? renderCalendar() : activeTab === 'ideas' ? <div className="ws-page"><header className="ws-page-heading"><div><span>{lang === 'uk' ? 'Бібліотека' : 'Library'}</span><h1>{lang === 'uk' ? 'Ідеї' : 'Ideas'}</h1></div></header>{renderIdeas()}</div> : activeTab === 'files' || activeTab === 'notes' ? <div className="ws-page"><header className="ws-page-heading"><div><span>{lang === 'uk' ? 'Бібліотека' : 'Library'}</span><h1>{lang === 'uk' ? 'Файли' : 'Files'}</h1></div></header>{renderFiles()}</div> : renderToday();

  return <div className="workspace-v2">
    <aside className="ws-sidebar"><div className="ws-brand"><img src="/icons/marketing-plan-192.png" alt="" /><span><strong>Marketing</strong><small>Workspace</small></span></div><nav><p>WORK</p><button className={activeTab === 'home' ? 'active' : ''} onClick={() => navigate('home')}><Home />{lang === 'uk' ? 'Сьогодні' : 'Today'}{dueToday.length ? <b>{dueToday.length}</b> : null}</button><button className={activeTab === 'plans' ? 'active' : ''} onClick={() => { setProjectOpen(false); navigate('plans'); }}><FolderKanban />{lang === 'uk' ? 'Проєкти' : 'Projects'}</button><button className={activeTab === 'all_tasks' ? 'active' : ''} onClick={() => navigate('all_tasks')}><ListTodo />{lang === 'uk' ? 'Завдання' : 'Tasks'}{overdue.length ? <b className="danger">{overdue.length}</b> : null}</button><button className={activeTab === 'content' ? 'active' : ''} onClick={() => navigate('content')}><LayoutDashboard />{lang === 'uk' ? 'Контент' : 'Content'}{contentItems.filter(item => item.status === 'review').length ? <b>{contentItems.filter(item => item.status === 'review').length}</b> : null}</button><p>INSIGHTS</p><button className={activeTab === 'analytics' ? 'active' : ''} onClick={() => navigate('analytics')}><ChartNoAxesCombined />{lang === 'uk' ? 'Аналітика' : 'Analytics'}</button><p>LIBRARY</p><button className={activeTab === 'ideas' ? 'active' : ''} onClick={() => navigate('ideas')}><Lightbulb />{lang === 'uk' ? 'Ідеї' : 'Ideas'}</button><button className={activeTab === 'files' || activeTab === 'notes' ? 'active' : ''} onClick={() => navigate('files')}><FileText />{lang === 'uk' ? 'Файли' : 'Files'}</button></nav><div className="ws-sidebar-bottom"><button className={activeTab === 'settings' ? 'active' : ''} onClick={() => navigate('settings')}><Settings />{lang === 'uk' ? 'Налаштування' : 'Settings'}</button><div className="ws-account"><span>{accountEmail.slice(0, 2).toUpperCase()}</span><div><strong>{accountEmail.split('@')[0]}</strong><small>{accountEmail}</small></div></div></div></aside>
    <main className="ws-main"><header className="ws-topbar"><button className="ws-mobile-menu" onClick={() => setMoreOpen(true)}><Menu /></button><button className="ws-project-picker" onClick={() => { setProjectOpen(false); navigate('plans'); }}>{activeProjectTitle}<ChevronDown size={15} /></button><button className="ws-global-search" onClick={() => setSearchOpen(true)}><Search size={16} /><span>{lang === 'uk' ? 'Знайти будь-що…' : 'Find anything…'}</span><kbd><Command size={12} />K</kbd></button><button className="ws-icon-button" onClick={onOpenReminders} aria-label={lang === 'uk' ? 'Нагадування' : 'Reminders'}><Bell />{remindersCount ? <b>{remindersCount}</b> : null}</button><div className="ws-quick-wrap"><button className="ws-primary ws-global-add" onClick={() => setQuickOpen(value => !value)}><Plus />{lang === 'uk' ? 'Додати' : 'Add'}</button>{quickOpen ? <div className="ws-popover ws-quick-menu"><button onClick={() => { setQuickOpen(false); onCreateTask(); }}><ListTodo />{lang === 'uk' ? 'Завдання' : 'Task'}</button><button onClick={() => { setQuickOpen(false); navigate('content'); }}><Clapperboard />{lang === 'uk' ? 'Контент' : 'Content'}</button><button onClick={() => { setQuickOpen(false); navigate('ideas'); }}><Lightbulb />{lang === 'uk' ? 'Ідею' : 'Idea'}</button><button onClick={() => { setQuickOpen(false); fileInputRef.current?.click(); }}><Upload />{lang === 'uk' ? 'Файл' : 'File'}</button></div> : null}</div></header><div className="ws-content">{content}</div></main>
    <nav className="ws-mobile-nav"><button className={activeTab === 'home' ? 'active' : ''} onClick={() => navigate('home')}><Home /><span>{lang === 'uk' ? 'Сьогодні' : 'Today'}</span></button><button className={activeTab === 'plans' ? 'active' : ''} onClick={() => { setProjectOpen(false); navigate('plans'); }}><FolderKanban /><span>{lang === 'uk' ? 'Проєкти' : 'Projects'}</span></button><button className="ws-mobile-add" onClick={() => setQuickOpen(value => !value)}><Plus /></button><button className={activeTab === 'content' ? 'active' : ''} onClick={() => navigate('content')}><LayoutDashboard /><span>{lang === 'uk' ? 'Контент' : 'Content'}</span></button><button onClick={() => setMoreOpen(true)}><MoreHorizontal /><span>{lang === 'uk' ? 'Ще' : 'More'}</span></button></nav>
    {quickOpen ? <div className="ws-mobile-quick"><button onClick={() => { setQuickOpen(false); onCreateTask(); }}><ListTodo />{lang === 'uk' ? 'Нове завдання' : 'New task'}</button><button onClick={() => { setQuickOpen(false); navigate('content'); }}><LayoutDashboard />{lang === 'uk' ? 'Додати контент' : 'Add content'}</button><button onClick={() => { setQuickOpen(false); navigate('ideas'); }}><Lightbulb />{lang === 'uk' ? 'Записати ідею' : 'Capture idea'}</button><button onClick={() => { setQuickOpen(false); fileInputRef.current?.click(); }}><Upload />{lang === 'uk' ? 'Завантажити файл' : 'Upload file'}</button></div> : null}
    {moreOpen ? <><button className="ws-overlay" onClick={() => setMoreOpen(false)} aria-label="Close" /><aside className="ws-more-sheet"><header><div><strong>{lang === 'uk' ? 'Ще' : 'More'}</strong><small>{accountEmail}</small></div><button onClick={() => setMoreOpen(false)}><X /></button></header><nav><button onClick={() => navigate('all_tasks')}><ListTodo />{lang === 'uk' ? 'Завдання' : 'Tasks'}</button><button onClick={() => navigate('analytics')}><ChartNoAxesCombined />{lang === 'uk' ? 'Аналітика' : 'Analytics'}</button><button onClick={() => navigate('calendar')}><CalendarDays />{lang === 'uk' ? 'Календар' : 'Calendar'}</button><button onClick={() => navigate('ideas')}><Lightbulb />{lang === 'uk' ? 'Ідеї' : 'Ideas'}</button><button onClick={() => navigate('files')}><FileText />{lang === 'uk' ? 'Файли' : 'Files'}</button><button onClick={() => navigate('settings')}><Settings />{lang === 'uk' ? 'Налаштування' : 'Settings'}</button><button onClick={onOpenTeam}><Users />{lang === 'uk' ? 'Команда' : 'Team'}</button><button onClick={onOpenReminders}><Bell />{lang === 'uk' ? 'Нагадування' : 'Reminders'}</button><button onClick={onOpenArchive}><Archive />{lang === 'uk' ? 'Архів' : 'Archive'}</button><button onClick={onToggleTheme}>{theme === 'light' ? <Moon /> : <Sun />}{lang === 'uk' ? 'Тема' : 'Theme'}</button><button onClick={onToggleLanguage}><Languages />{lang === 'uk' ? 'English' : 'Українська'}</button><button onClick={onSignOut}><LogOut />{lang === 'uk' ? 'Вийти' : 'Sign out'}</button></nav></aside></> : null}
