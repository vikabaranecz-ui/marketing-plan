import { Archive, Bell, CalendarRange, ChevronRight, Pencil, Plus, Trash2 } from 'lucide-react';
...
export default function MobilePlansView({ plans, activePlanId, lang, onAdd, onArchive, onDelete, onOpen, onRename, onReminder }: MobilePlansViewProps) {
  return (
        ...
        {plans.map(plan => (
          <article className={`mobile-plan-card ${plan.id === activePlanId ? 'active' : ''}`} key={plan.id}>
            ...
            <div className="mobile-plan-actions">
              ...
              <button className="danger" onClick={() => onDelete(plan.id)}><Trash2 size={14} />{lang === 'uk' ? 'Видалити' : 'Delete'}</button>
            </div>
          </article>
        ))}
      </div>
